...
class NotificationTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @env.enable_plugin('NotificationPlugin')

    @user = User.create!(:environment_id => @env.id, :email => "user@domain.com", :login   => "new_user", :password => "test", :password_confirmation => "test")
    @danger_notification = NotificationPlugin::DangerNotification.create!(
                      :target => @env,
                      :message => "Danger Message",
                      :active => true,
                    )

    @warning_notification = NotificationPlugin::WarningNotification.create!(
                      :target => @env,
                      :message => "Warning Message",
                      :active => true,
                    )

    @information_notification = NotificationPlugin::InformationNotification.create!(
                      :target => @env,
  ...
  should 'get all notifications that a user did not close' do
    ...
    notifications = NotificationPlugin::Notification.visibles(@env, @user, nil)
  ...
  should 'get only notifications configured to be displayed to all users' do
    ...
    notifications = NotificationPlugin::Notification.visibles(@env, nil, nil)
  ...
  should 'get only notifications configured to be displayed to all users and in all pages' do
    ...
    notifications = NotificationPlugin::Notification.visibles(@env, nil, 'not_home')
  ...
  should 'get only notifications configured to be displayed in all pages' do
    ...
    notifications = NotificationPlugin::Notification.visibles(@env, @user, "not_home")

    assert !notifications.include?(@danger_notification)
    assert notifications.include?(@warning_notification)
    assert notifications.include?(@information_notification)

    notifications = NotificationPlugin::Notification.visibles(@env, nil, "home")
  ...
  should 'get notifications configured to be displayed on profile' do
    ...
    NotificationPlugin::Notification.destroy_all
    ...
    notifications = NotificationPlugin::Notification.visibles(community, @user, "profile")
    assert_equivalent notifications.to_a, [env_not_home_notification, profile_not_home_notification, profile_home_notification]

    notifications = NotificationPlugin::Notification.visibles(community, @user, "profile_but_bot_homepage")
  ...
  should 'get notifications configured to be displayed on environment' do
    ...
    notifications = NotificationPlugin::Notification.visibles(@env, @user, "home")
    assert_equivalent notifications.to_a, [env_home_notification, env_not_home_notification]

    notifications = NotificationPlugin::Notification.visibles(@env, @user, "not_home_not_profile")
  ...
  should 'get only notifications with popup' do
    ...
    notifications = NotificationPlugin::Notification.with_popup(@env, @user, 'home')

    assert !notifications.include?(@danger_notification)
    assert !notifications.include?(@warning_notification)
    assert notifications.include?(@information_notification)

    notifications = NotificationPlugin::Notification.with_popup(@env, nil, nil)
