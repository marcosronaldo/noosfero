class NotificationPlugin::Notification < ActiveRecord::Base

  self.table_name = "notification_plugin_notifications"

  TYPE_LIST = [
    "NotificationPlugin::WarningNotification",
    "NotificationPlugin::SuccessNotification",
    "NotificationPlugin::InformationNotification",
    "NotificationPlugin::DangerNotification"
  ]
  ...
  def self.visibles(target, user, controller_path)
    notifications = NotificationPlugin::Notification.active(target).order('updated_at DESC')
  ...
  def self.with_popup(target, user, previous_path)
    NotificationPlugin::Notification.visibles(target, user, previous_path).where(display_popup: true)
  end
